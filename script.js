...
$('main').append(data.map(([title, elements], index) => `
	<div>
		...
		<div class="grid items-center justify-center gap-5 mb-32 px-20" style="grid-template-columns: repeat(auto-fill, minmax(18em, 1fr))">
			${elements.map(([img, name, level, time, maxVal, mats, used, desc]) => `
				<div class="bg-white rounded-xl flex flex-col items-center p-6 relative" style="box-shadow: 0 4px 4px rgba(0, 0, 0, .25); height: ${elements.map(e => e[5].length > 0).some(e => e) ? "40em" : "28em"}">
					...
					${elements.map(e => mats.length > 0).some(e => e) ? `
						<h3 class="font-extrabold text-lg mb-3 mt-8">Materials</h3>
						<div class="flex">
							${mats.map(([i, a, n]) => `
							<div class="flex items-center justify-center w-12 h-12 mx-1 rounded-full bg-${COLORMAP[getCMAPIndex(n)]}" style="box-shadow: inset 0 2px 6px rgba(0, 0, 0, .6)"><img src="${DOMAIN+i}" class="bg-white rounded-full p-2 w-10 h-10" style="box-shadow: 0 2px 4px rgba(0, 0, 0, .6)"></div>
							`).join("")}
						</div>` : ""}
					${elements.map(e => used.length > 0).some(e => e) ? `
